...
This is the local, self-hosted equivalent of frontier "sub-agent orchestration" features (Claude Code's Agent tool, OpenAI AgentKit). Orchestration is handled by [Microsoft AutoGen](https://microsoft.github.io/autogen/) (`autogen-agentchat` >= 0.4): each team member becomes an `AssistantAgent` that talks to your **local Ollama** server through AutoGen's Ollama model client, with its allowed tools exposed as functions.

### Installing the engine

AutoGen is an **optional extra** so the serverless core stays light:

```bash
pip install "local-mcp[agents-autogen]"
```

Without it, `run_agent_team` raises an error telling you to run that command. (The text-only fallback for `LLM_PROVIDER=gemini`, described below, still works without AutoGen.)
...
Tools are exposed to each `AssistantAgent` as functions and driven through the Ollama model client, so tool calling requires `LLM_PROVIDER=ollama` (the default) and a model that supports tools (for example `qwen2.5`, `qwen3`, `llama3.1+`). With `LLM_PROVIDER=gemini` teams still run, but text-only through `llm.generate_text` (no AutoGen needed): tool allowlists are dropped with a note in the run summary. Tool failures are returned to the agent as text (never crash the run), each result is truncated to `LOCAL_MCP_AGENT_TOOL_RESULT_CHARS` (default 8000), and each agent has a per-run budget of `max_tool_calls` (default `LOCAL_MCP_AGENT_MAX_TOOL_CALLS`, 6).
