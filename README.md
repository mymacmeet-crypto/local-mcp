...
Sub-agent orchestration: run a small team of role-based agents (2-3 roles work best on a local model) sequentially on one task. Each agent has its own system-prompt role and an optional tool allowlist (`web_search`, `web_fetch`, `extract_urls`, `parse_document`, driven through Ollama function calling); each agent reads the task plus the previous agents' hand-offs, and the last agent's message is the team's final answer.
